...
from .utils.cond_diff_denoise import Cond_Diff_Denoise
tv = None
try:
...
class PointPillar_InterF(nn.Module):
    def __init__(self, cfg):
        ...
        try:
            self.cdd = Cond_Diff_Denoise(self.model_cfg.CDD_block, 64)
        except:
            pass
        # class
        self.num_class = 0
    ...
    def pre_processor(self, batch_dict):
        if self.is_pre_processing is None:
            ...
            if 'de_ldr64' in batch_dict.keys():
                batched_ldr64 = batch_dict['de_ldr64']
                batched_indices_ldr64 = batch_dict['batch_indices_de_ldr64']
                list_points = []
                list_voxels = []
                list_voxel_coords = []
                list_voxel_num_points = []
                ...
                batched_points = torch.cat(list_points, dim=0)
                batch_dict['de_lidar_points'] = torch.cat((batched_indices_ldr64.reshape(-1,1), batched_points), dim=1).cuda() # b, x, y, z, intensity
                
                batch_dict['de_lidar_voxels'] = torch.from_numpy(np.concatenate(list_voxels, axis=0)).cuda()
                batch_dict['de_lidar_voxel_coords'] = torch.from_numpy(np.concatenate(list_voxel_coords, axis=0)).cuda()
                batch_dict['de_lidar_voxel_num_points'] = torch.from_numpy(np.concatenate(list_voxel_num_points, axis=0)).cuda()
            return batch_dict
    ...
    def forward(self, batch_dict):
        ...
        if 'de_lidar_spatial_features' in batch_dict.keys():
            batch_dict['de_spatial_features'] = batch_dict['de_lidar_spatial_features']
            batch_dict['ra_spatial_features'] = batch_dict['radar_spatial_features']
            batch_dict['bf_spatial_features'] = batch_dict['spatial_features'] = batch_dict['lidar_spatial_features']
            batch_dict['train'] = self.training
            
            batch_dict = self.cdd(batch_dict)
            batch_dict['lidar_spatial_features'] = batch_dict['pred_feature'] * (batch_dict['lidar_spatial_features'] != 0)
            #vis_feature_denoise(lidar_batch_dict)
            batch_dict.update({'pred_feature' : batch_dict['lidar_spatial_features'],
                        'gt_feature' : batch_dict['de_spatial_features']})
            batch_dict['spatial_features'] = torch.cat([batch_dict['lidar_spatial_features'], batch_dict['radar_spatial_features']],dim = 1)
    ...
    def loss(self, dict_item):
        ...
        self.lossrpn += loss_rpn
        if self.point_head is not None:
            loss_point, tb_dict = self.point_head.get_loss(tb_dict)
        else:
            loss_point = 0
        if 'pred_feature' in dict_item.keys():
            p_e = dict_item['pred_feature']
            g_e = dict_item['gt_feature']
            diff_loss = self.diff_loss_func(p_e,
                                g_e,
                                weights=self.sigmoid_weight(3, dict_item['epoch']))
            self.lossdiff += diff_loss
        else:
            diff_loss = 0 
        loss = loss_rpn + loss_point + diff_loss
        if self.is_logging:
            dict_item['logging'] = dict()
            dict_item['logging'].update(tb_dict)
        
        
        if self.cnt % 500 == 0 and self.cnt != 0:
            with open(dict_item['path'], 'a') as file:
                file.write('At epoch %d , the RPN loss is %.4f  the MDFD loss is %.4f \n' % (dict_item['epoch'], self.lossrpn / self.cnt , self.lossdiff / self.cnt))
            self.lossdiff = 0
            self.lossrpn = 0
            self.cnt = 0
        self.cnt += 1
