...
class PointPillar_RADAR(nn.Module):
    def __init__(self, cfg):
        ...
        # Common params
        num_point_features = self.dataset_cfg.rdr_sparse.n_used
    ...
    def pre_processor(self, batch_dict):
        if self.is_pre_processing is None:
            ...
            batched_rdr= batch_dict['rdr_sparse']
            batched_indices_rdr= batch_dict['batch_indices_rdr_sparse']
            list_points = []
            list_voxels = []
            list_voxel_coords = []
            list_voxel_num_points = []
            for batch_idx in range(batch_dict['batch_size']):
                temp_points = batched_rdr[torch.where(batched_indices_rdr == batch_idx)[0],:self.num_point_features]
                ...
                if self.transform_points_to_voxels:
                    if self.training:
                        voxels, coordinates, num_points = self.voxel_generator_train.generate(temp_points.cpu().numpy())
                    else:
                        voxels, coordinates, num_points = self.voxel_generator_train.generate(temp_points.cpu().numpy())
            ...
            batch_dict['points'] = torch.cat((batched_indices_rdr.reshape(-1,1), batched_points), dim=1).cuda() # b, x, y, z, intensity
