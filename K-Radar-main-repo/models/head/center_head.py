...
class Batch_CenterHead(nn.Module):
    ...
    def generate_predicted_boxes(self, batch_size, pred_dicts):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            for k, final_dict in enumerate(final_pred_dicts):
                final_dict['batch_pred_labels'] = self.class_id_mapping_each_head[idx][final_dict['batch_pred_labels'].long()]
                ...
                if post_process_cfg.get('USE_IOU_TO_RECTIFY_SCORE', False) and 'pred_iou' in final_dict:
                    ...
                    IOU_RECTIFIER = final_dict['batch_pred_scores'].new_tensor(post_process_cfg.IOU_RECTIFIER)
                    final_dict['batch_pred_scores'] = torch.pow(final_dict['batch_pred_scores'], 1 - IOU_RECTIFIER[final_dict['batch_pred_labels']]) * torch.pow(pred_iou, IOU_RECTIFIER[final_dict['batch_pred_labels']])

                if post_process_cfg.NMS_CONFIG.NMS_TYPE not in  ['circle_nms', 'class_specific_nms']:
                    selected, selected_scores = model_nms_utils.class_agnostic_nms(
                        box_scores=final_dict['batch_pred_scores'], box_preds=final_dict['batch_pred_boxes'],
                    ...
                    selected, selected_scores = model_nms_utils.class_specific_nms(
                        box_scores=final_dict['batch_pred_scores'], box_preds=final_dict['batch_pred_boxes'],
                        box_labels=final_dict['batch_pred_labels'], nms_config=post_process_cfg.NMS_CONFIG,
                        score_thresh=post_process_cfg.NMS_CONFIG.get('SCORE_THRESH', None)
                    )
                elif post_process_cfg.NMS_CONFIG.NMS_TYPE == 'circle_nms':
                    raise NotImplementedError

                final_dict['batch_box_preds'] = final_dict['batch_pred_boxes'][selected]
                final_dict['batch_pred_scores'] = selected_scores
                final_dict['batch_pred_labels'] = final_dict['batch_pred_labels'][selected]

                ret_dict[k]['batch_box_preds'].append(final_dict['batch_box_preds'])
                ret_dict[k]['batch_pred_scores'].append(final_dict['batch_pred_scores'])
                ret_dict[k]['batch_pred_labels'].append(final_dict['batch_pred_labels'])

        for k in range(batch_size):
            ret_dict[k]['batch_box_preds'] = torch.cat(ret_dict[k]['batch_box_preds'], dim=0)
            ret_dict[k]['batch_pred_scores'] = torch.cat(ret_dict[k]['batch_pred_scores'], dim=0)
            ret_dict[k]['batch_pred_labels'] = torch.cat(ret_dict[k]['batch_pred_labels'], dim=0) + 1
        ret_dict['batch_box_preds'] = torch.cat(ret_dict[:]['batch_box_preds'], dim=0)
        ret_dict['batch_pred_scores'] = torch.cat(ret_dict[:]['batch_pred_scores'], dim=0)
        ret_dict['batch_pred_labels'] = torch.cat(ret_dict[:]['batch_pred_labels'], dim=0)
        return ret_dict

    @staticmethod
    def reorder_rois_for_refining(batch_size, pred_dicts):
        num_max_rois = max([len(cur_dict['batch_box_preds']) for cur_dict in pred_dicts])
        num_max_rois = max(1, num_max_rois)  # at least one faked rois to avoid error
        pred_boxes = pred_dicts[0]['batch_box_preds']
        ...
        for bs_idx in range(batch_size):
            num_boxes = len(pred_dicts[bs_idx]['batch_box_preds'])

            rois[bs_idx, :num_boxes, :] = pred_dicts[bs_idx]['batch_box_preds']
            roi_scores[bs_idx, :num_boxes] = pred_dicts[bs_idx]['batch_pred_scores']
            roi_labels[bs_idx, :num_boxes] = pred_dicts[bs_idx]['batch_pred_labels']
