...
def random_flip_along_x(gt_boxes, lidar_points, radar_points = None, bfgt = None):
    """
    Args:
    ...
    if radar_points is not None:
        if bfgt is not None:
            return gt_boxes, lidar_points, radar_points, bfgt
        return gt_boxes, lidar_points, radar_points
    if bfgt is not None:
        return gt_boxes, lidar_points, bfgt
    return gt_boxes, lidar_points


def random_flip_along_y(gt_boxes, lidar_points, radar_points = None, return_flip=False, enable=None):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C)
...
def global_rotation(gt_boxes, lidar_points, radar_points=None, bfgt = None, rot_range = 1 , return_rot=False, noise_rotation=None):
    """
    Args:
        gt_boxes: (N, 7 + C), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
    ...
    if bfgt is not None:
        bfgt[:, 0:3] = common_utils.rotate_points_along_z(bfgt[np.newaxis, :, 0:3], np.array([noise_rotation]))[0]
        bfgt[:, 6] += noise_rotation
    if gt_boxes.shape[1] > 7:
...
def global_scaling(gt_boxes, lidar_points, radar_points = None, bfgt = None, scale_range = 1, return_scale=False):
    """
    Args:
