...
class DFusion_PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        ...
        # Radar : x y z cx cy cz dx dy dz V Vcom R t Score
        # Fusion : x y z Lcx Lcy Lcz mx my mz I V Vcom R t Score
        num_point_features = num_point_features_l + num_point_features_r - 9
        if not self.use_preground_score:
            num_point_features = num_point_features - 1 
        self.num_point_features = num_point_features
        print("lidar feature dim = ",num_point_features_l)
        print("radar feature dim = ",num_point_features_r)
        print("common feature dim = ",num_point_features)
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        # com_features[l2r_mask, :, now_feature_idx :] = extraFea_R #4
        ...
        for i in range(5):
            ...
            #radar特征部分修改
            com_features[valid_mask, replaced_idx, now_feature_idx :] = radar_voxel_features[valid_common_R, i, 3 :]
            now_feature_idx += radar_voxel_features.shape[-1] - 3
            #print(now_feature_idx)
        ...
        # if len(common_L) > 0:
        #     if len(radar_features.shape) == 1:
        #         radar_features = torch.unsqueeze(radar_features, -1)
        #     T = 10
        ...
        #     eps = 1e-6
        #     com_weight = torch.tanh(min_pillar_num / (dif_pillar_num + eps)).reshape(-1, 1)
        ...
        #     lidar_features[common_L, :] = lidar_features[common_L, :] * LR_pillar_weight[:, 0].reshape(-1, 1)
        #     radar_features[common_R, :] = radar_features[common_R, :] * LR_pillar_weight[:, 1].reshape(-1, 1)
        radar_features = radar_features.squeeze()
