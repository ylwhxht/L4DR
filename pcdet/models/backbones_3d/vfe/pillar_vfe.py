...
class BiDFCA_PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        super().__init__(model_cfg=model_cfg)
        self.use_norm = self.model_cfg.USE_NORM
        self.with_distance = self.model_cfg.WITH_DISTANCE
        ...
        self.r2l_attention_layers = Attention_Layer(list(self.num_filters)[-1], 32)
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        for pfn in self.r_pfn_layers:
            radar_features = pfn(radar_features)
        radar_features = radar_features.squeeze()
        ca_radar_features = self.l2r_attention_layers(
                lidar_features, 
                radar_features, 
                lidar_coords,
                radar_coords
            )
        ca_lidar_features = self.r2l_attention_layers(
                ca_radar_features,
                lidar_features, 
                radar_coords,
                lidar_coords,
            )
        batch_dict['lidar_pillar_features'] = ca_lidar_features
        batch_dict['radar_pillar_features'] = ca_radar_features
