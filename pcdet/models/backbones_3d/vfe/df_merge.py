...
class DFusion_PillarVFE_Merge(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        super().__init__(model_cfg=model_cfg)
        self.use_norm = self.model_cfg.USE_NORM
        self.with_distance = self.model_cfg.WITH_DISTANCE
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        final_features = torch.zeros((len(L_coords)+len(only_R), 32, self.num_point_features)).cuda()
        ###########################原本的Lidar Voxel部分########################################
        ###########################redesign : Common Lidar部分##################################
        # x y z I cluster_lidar center cluster_radar V R t 
        now_feature_idx = 0
        final_features[:len(L_coords), :, now_feature_idx : now_feature_idx + lidar_voxel_features.shape[-1]] = lidar_voxel_features #4
        now_feature_idx += lidar_voxel_features.shape[-1]

        final_features[:len(L_coords), :, now_feature_idx : now_feature_idx + lidar_f_cluster.shape[-1]] = lidar_f_cluster #3
        now_feature_idx += lidar_f_cluster.shape[-1]

        final_features[:len(L_coords), :, now_feature_idx : now_feature_idx + lidar_f_center.shape[-1]] = lidar_f_center #3
        ...
        final_features[l2r_com_L, :, now_feature_idx : now_feature_idx + lidartoradar_f_cluster.shape[-1]] = lidartoradar_f_cluster #3
        now_feature_idx += lidartoradar_f_cluster.shape[-1]


        #radar特征部分先填充均值后面是radar的话会覆盖（没有的部分默认填充0）
        ...
        final_features[l2r_com_L, :, now_feature_idx : now_feature_idx + extraFea_R.shape[-1]] = extraFea_R #4
        
        now_feature_idx += extraFea_R.shape[-1]
        
        
        #每个体素内额外的新加入的点数
        additional_points = torch.zeros((len(L_coords)+len(only_R))).cuda()
        ...
        for i in range(5):
            ...
            additional_points[valid_common_L] += 1
            ...
            final_features[valid_common_L, replaced_idx , now_feature_idx : now_feature_idx + 3] = radar_voxel_features[valid_common_R, i, :3] #3
            now_feature_idx += 3
            ...
            final_features[valid_common_L, replaced_idx, now_feature_idx : now_feature_idx + 1] = extraF_L #1
            now_feature_idx += 1
            
            # final_features[valid_common_L, replaced_idx, now_feature_idx : now_feature_idx + 1] = 0 #1
            # now_feature_idx += 1
            ...
            final_features[valid_common_L, replaced_idx, now_feature_idx : now_feature_idx + radartolidar_f_cluster.shape[-1]] = radartolidar_f_cluster #3
            now_feature_idx += radartolidar_f_cluster.shape[-1]
            
            final_features[valid_common_L, replaced_idx, now_feature_idx : now_feature_idx + radar_f_center.shape[-1]] = radar_f_center[valid_common_R, i] #3
            now_feature_idx += radar_f_center.shape[-1]

            final_features[valid_common_L, replaced_idx, now_feature_idx : now_feature_idx + radar_f_cluster.shape[-1]] = radar_f_cluster[valid_common_R, i] #3
            now_feature_idx += radar_f_cluster.shape[-1]

            
            #radar特征部分修改
            final_features[valid_common_L, replaced_idx, now_feature_idx : now_feature_idx + radar_voxel_features.shape[-1] - 3] = radar_voxel_features[valid_common_R, i, 3:]
            now_feature_idx += radartolidar_f_cluster.shape[-1]
        
        ###########################原本的Radar Voxel部分########################################
        now_feature_idx = 0
        final_features[len(L_coords):, :, now_feature_idx : now_feature_idx + 3] = radar_voxel_features[only_R, :, :3] #3
        now_feature_idx += 3
        ...
        #radar 体素center 
        final_features[len(L_coords):, :, now_feature_idx : now_feature_idx + radar_f_center.shape[-1]] = radar_f_center[only_R] #3
        now_feature_idx += radar_f_center.shape[-1]

        #到radar 点中心
        final_features[len(L_coords):, :, now_feature_idx : now_feature_idx + radar_f_cluster.shape[-1]] = radar_f_cluster[only_R] #3
        now_feature_idx += radar_f_cluster.shape[-1]

        #radar特征部分
        final_features[len(L_coords):, :, now_feature_idx : now_feature_idx + radar_voxel_features.shape[-1]] = radar_voxel_features[only_R, :, 3:] #3
        now_feature_idx += radar_voxel_features.shape[-1]

        #创建新的voxel_num_points和coords
        voxel_num_points = torch.cat([lidar_voxel_num_points, radar_voxel_num_points[only_R]]).cuda()
        voxel_coords = torch.cat([lidar_coords, radar_coords[only_R]]).cuda()
        voxel_num_points += additional_points
        voxel_num_points[voxel_num_points > 32] = 32
        ...
        final_voxel_count = final_features.shape[1]
        mask = self.get_paddings_indicator(voxel_num_points, final_voxel_count, axis=0)
        mask = torch.unsqueeze(mask, -1).type_as(final_features)
        final_features *= mask

        for pfn in self.pfn_layers:
            final_features = pfn(final_features)
        final_features = final_features.squeeze()
        
        
        batch_dict['pillar_features'] = final_features
        return batch_dict
