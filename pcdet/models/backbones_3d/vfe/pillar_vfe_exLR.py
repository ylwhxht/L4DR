...
class DFusion_PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        ...
        com_num_point_features = num_point_features_l + num_point_features_r - 6
        self.num_point_features_r = com_num_point_features
        self.num_point_features_l = com_num_point_features
        print("common feature dim (use preground_score) = ", com_num_point_features)


        self.num_filters = self.model_cfg.NUM_FILTERS
        assert len(self.num_filters) > 0
        num_filters = [com_num_point_features] + list(self.num_filters)
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        com_features_l = torch.zeros((len(lidar_voxel_num_points), lidar_voxel_features.shape[1], self.num_point_features_l)).cuda()
        ###########################原本的Lidar Voxel部分########################################
        ###########################redesign : Common Lidar部分##################################
        # x y z I cluster_lidar center cluster_radar V R t 
        now_feature_idx = 0
        com_features_l[:, :, now_feature_idx : now_feature_idx + lidar_voxel_features.shape[-1]] = lidar_voxel_features #4
        now_feature_idx += lidar_voxel_features.shape[-1]

        com_features_l[:, :, now_feature_idx : now_feature_idx + lidar_f_cluster.shape[-1]] = lidar_f_cluster #3
        now_feature_idx += lidar_f_cluster.shape[-1]

        com_features_l[:, :, now_feature_idx : now_feature_idx + lidar_f_center.shape[-1]] = lidar_f_center #3
        ...
        com_features_l[l2r_com_L, :, now_feature_idx : now_feature_idx + lidartoradar_f_cluster.shape[-1]] = lidartoradar_f_cluster #3
        ...
        com_features_l[l2r_com_L, :, now_feature_idx : now_feature_idx + extraFea_R.shape[-1]] = extraFea_R #4
        ...
        com_features_r = torch.zeros((len(radar_voxel_num_points), len_radar, self.num_point_features_r)).cuda()
        ...
        for i in range(len_radar):
            ...
            com_features_r[:, i, now_feature_idx : now_feature_idx + 3] = radar_voxel_features[:, i, :3] #3
            now_feature_idx += 3
            ...
            com_features_r[valid_common_R, i, now_feature_idx : now_feature_idx + 1] = extraF_L #1
            now_feature_idx += 1
            
            # com_features_r[valid_common_L, replaced_idx, now_feature_idx : now_feature_idx + 1] = 0 #1
            # now_feature_idx += 1
            ...
            com_features_r[valid_common_R, i, now_feature_idx : now_feature_idx + radartolidar_f_cluster.shape[-1]] = radartolidar_f_cluster #3
            now_feature_idx += radartolidar_f_cluster.shape[-1]
            
            com_features_r[:, i, now_feature_idx : now_feature_idx + radar_f_center.shape[-1]] = radar_f_center[:, i] #3
            now_feature_idx += radar_f_center.shape[-1]

            com_features_r[:, i, now_feature_idx : now_feature_idx + radar_f_cluster.shape[-1]] = radar_f_cluster[:, i] #3
            now_feature_idx += radar_f_cluster.shape[-1]

            
            #radar特征部分修改
            com_features_r[:, i, now_feature_idx : now_feature_idx + radar_voxel_features.shape[-1] - 3] = radar_voxel_features[:, i, 3:]
            now_feature_idx += radartolidar_f_cluster.shape[-1]
        
        lidar_features = com_features_l
...
class DFA_PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        ...
        print("common feature dim (use preground_score) = ",num_point_features)

        self.num_filters_com = self.model_cfg.NUM_FILTERS_COM
        assert len(self.num_filters_com) > 0
        num_filters = [num_point_features] + list(self.num_filters_com)
        ...
        self.num_filters_l = self.model_cfg.NUM_FILTERS
        assert len(self.num_filters_l) > 0
        num_filters = [num_point_features_l] + list(self.num_filters_l)
        ...
        self.num_filters_r = self.model_cfg.NUM_FILTERS_Radar
        assert len(self.num_filters_r) > 0
        num_filters = [num_point_features_r] + list(self.num_filters_r)
        ...
        self.r2l_attention_layers = Attention_Layer(list(self.num_filters_r)[-1], list(self.num_filters_l)[-1])
        self.l2r_attention_layers = Attention_Layer(list(self.num_filters_l)[-1], list(self.num_filters_r)[-1])
        
    def get_output_feature_dim(self):
        return self.num_filters_l[-1]
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        com_features = com_features.squeeze().view(-1, com_features.shape[-1])
        ...
        for pfn in self.l_pfn_layers:
            lidar_features = pfn(lidar_features)
        lidar_features = lidar_features.squeeze().view(-1, lidar_features.shape[-1])
        ...
        for pfn in self.r_pfn_layers:
            radar_features = pfn(radar_features)
        radar_features = radar_features.squeeze().view(-1, radar_features.shape[-1])
        ...
        #lidar_features = torch.cat([lidar_features, lidar_attentive_fuse_feat], dim=-1)
        #radar_features = torch.cat([radar_features, radar_attentive_fuse_feat], dim=-1)
        # lidar_features = lidar_features + lidar_attentive_fuse_feat
        # radar_features = radar_features + radar_attentive_fuse_feat
...
class Attention_Layer(nn.Module):
    def t__init__(self, hidden_dim_list, head=4):
        super(Attention_Layer, self).__init__()
        align_hidden_dim = 64
        self.Align_linear = nn.ModuleList()
        self.Align_idx = []
        self.deAlign_linear = nn.ModuleList()
        ...
        for idx, hidden_dim in enumerate(hidden_dim_list):
            if hidden_dim != align_hidden_dim:
                self.Align_linear.append(nn.Linear(hidden_dim, align_hidden_dim, bias=False))
                self.deAlign_linear.append(nn.Linear(align_hidden_dim, hidden_dim, bias=False))
                self.Align_idx.append(idx)
            self.Q_linear.append(nn.Linear(hidden_dim + self.pos_dim, align_hidden_dim, bias=False))
        self.K_linear = nn.Linear(align_hidden_dim + self.pos_dim, align_hidden_dim, bias=False)
        self.V_linear = nn.Linear(align_hidden_dim + self.pos_dim, align_hidden_dim, bias=False)
        self.att = nn.MultiheadAttention(align_hidden_dim, head, batch_first=True)
    

    def __init__(self, inputs_dim, Q_in_dim, head=4):
        super(Attention_Layer, self).__init__()
        self.hidden_dim = Q_in_dim
        self.pos_dim = 8
        self.pos_en = PositionalEmbedding(self.pos_dim)
        self.Q_linear = nn.Linear(Q_in_dim + self.pos_dim, Q_in_dim, bias=False)
        self.K_linear = nn.Linear(inputs_dim + self.pos_dim, Q_in_dim, bias=False)
        self.V_linear = nn.Linear(inputs_dim + self.pos_dim, Q_in_dim, bias=False)
        self.att = nn.MultiheadAttention(Q_in_dim, head, batch_first=True)
        self.drop = nn.Dropout(0.1)
        self.norm = nn.LayerNorm(Q_in_dim)
    ...
    def forward(self, inputs, Q_in, input_coords, Q_in_coords): # 
        ...
        for bs in range(batch_size):
            ...
            seq_len = inputs_bs.shape[1]
            pos_input = torch.from_numpy(np.arange(seq_len)+1).cuda()
            pos_input = self.pos_en(pos_input, 1)
            inputs_pos = torch.cat([inputs_bs, pos_input], -1)

            Q_in_bs = Q_in[:, Q_in_bs_mask, :]
            seq_len = Q_in_bs.shape[1]
            pos_input = torch.from_numpy(np.arange(seq_len)+1).cuda()
            pos_input = self.pos_en(pos_input, 1)
            Q_in_pos = torch.cat([Q_in_bs, pos_input], -1)
    ...
    def t_forward(self, inputs_list, coords_list): # B,K,N
        outs = []
        len_l, len_r, len_com = inputs_list[0].shape[1], inputs_list[1].shape[1], inputs_list[2].shape[1]

        feature = []
        batch_size = coords_list[0][:, 0].max().int().item() + 1
        fusion_inputs_list = []
        for i in range(len(inputs_list)):
            inputs_list[i] = inputs_list[i].reshape(1, -1, inputs_list[i].shape[-1])
            fusion_inputs_list.append(torch.zeros((1, inputs_list[i].shape[1], inputs_list[0].shape[-1])).cuda())

        for i, idx in enumerate(self.Align_idx):
            for bs in range(batch_size):
                bs_mask = coords_list[idx][:,0] == bs
                fusion_inputs_list[idx][:, bs_mask, :] = self.Align_linear[i](inputs_list[idx][:, bs_mask, :])

        for i in range(len(inputs_list)):
            inputs_list[i] = inputs_list[i].reshape(1, -1, inputs_list[i].shape[-1])
            feature.append(torch.zeros_like(fusion_inputs_list[i]).view(-1, inputs_list[0].shape[-1]))

        
        fusion_inputs = torch.cat(fusion_inputs_list, 1)
        fusion_coords = torch.cat(coords_list, 0)

        for bs in range(batch_size):
            bs_mask = fusion_coords[:,0] == bs
            fusion_inputs_bs = fusion_inputs[:, bs_mask, :]
            seq_len = fusion_inputs_bs.shape[1]
            fusion_pos_input = torch.from_numpy(np.arange(seq_len)+1).cuda()
            fusion_pos_input = self.pos_en(fusion_pos_input, 1)
            fusion_pos_input = torch.cat([fusion_inputs_bs, fusion_pos_input], -1)
            K = self.K_linear(fusion_pos_input)
            V = self.V_linear(fusion_pos_input)

            for m_idx in range(len(inputs_list)):
                inputs = inputs_list[m_idx]
                coords = coords_list[m_idx]
            
                bs_mask = coords[:,0] == bs
                inputs_bs = inputs[:, bs_mask, :]
                seq_len = inputs_bs.shape[1]
                pos_input = torch.from_numpy(np.arange(seq_len)+1).cuda()
                pos_input = self.pos_en(pos_input, 1)
                inputs_pos = torch.cat([inputs_bs, pos_input], -1)

                Q = self.Q_linear[m_idx](inputs_pos)
                feature[m_idx][bs_mask, :] = self.att(Q, K, V)[0].squeeze()

        for i, idx in enumerate(self.Align_idx):
            feature[idx] = self.deAlign_linear[i](feature[idx])

        for i in range(len(feature)):
            feature[i] = feature[i].reshape(-1, feature[i].shape[-1])
        return feature
