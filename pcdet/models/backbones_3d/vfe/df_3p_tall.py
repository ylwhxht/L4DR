...
class DFusion_PillarVFE(VFETemplate):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        common_radar_points_mean = radar_voxel_features[common_R, :, :3].sum(dim=1, keepdim=True) / radar_voxel_num_points[common_R].type_as(radar_voxel_features).view(-1, 1, 1)
        lidartoradar_f_cluster = lidar_voxel_features[common_L, :, :3] - common_radar_points_mean

        com_features[:, :, now_feature_idx : now_feature_idx + lidartoradar_f_cluster.shape[-1]] = lidartoradar_f_cluster #3
        now_feature_idx += lidartoradar_f_cluster.shape[-1]


        #radar特征部分先填充均值后面是radar的话会覆盖
        extraFea_R = radar_voxel_features[:, :, 3:].sum(dim=1, keepdim=True) / radar_voxel_num_points[common_R].type_as(radar_voxel_features).view(-1, 1, 1)
        com_features[:, :, now_feature_idx : now_feature_idx + extraFea_R.shape[-1]] = extraFea_R #4
