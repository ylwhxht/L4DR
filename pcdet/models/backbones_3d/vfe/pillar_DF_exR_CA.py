...
class DFA_PillarVFE(VFETemplate):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        #��radar�Ĳ��ָ��ǣ�radar��Ƚ��٣�һ��1~5�����5���㣬һ�δ�����  
        for i in range(len_radar):
            now_feature_idx = 0
            valid_mask = radar_voxel_num_points[common_R] >= i+1 #ֻ���Ƿǿյĵ�
            ...
            #Intensity ����Ϊ��ֵ��radar���ֵ�lidar��������Ϊ0��
        ...
        fog_I = batch_dict['fog_intensity']
...
class DFA_MA_PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        super().__init__(model_cfg=model_cfg)
        self.use_norm = self.model_cfg.USE_NORM
        self.with_distance = self.model_cfg.WITH_DISTANCE
        ...
        self.y_offset = self.voxel_y / 2 + point_cloud_range[1]
        self.z_offset = self.voxel_z / 2 + point_cloud_range[2]
        self.attention_layers = Attention_Layer(list(self.num_filters)[-1], 64)
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        # radar_features[common_R] = radar_features[common_R] * LR_pillar_weight[i][1]
        fog_I = batch_dict['fog_intensity']
        mix_features = torch.cat([lidar_features,radar_features], dim=0)
        mix_coords = torch.cat([lidar_coords,radar_coords], dim=0)
        radar_features = self.attention_layers(
            mix_features, 
            radar_features, 
            mix_coords,
            radar_coords
        ...
        mix_features = torch.cat([lidar_features, radar_features], dim=0)
        lidar_features = self.attention_layers(
            mix_features, 
            lidar_features, 
            mix_coords,
            lidar_coords
...
class Attention_Layer_EZPOS(nn.Module):
    def __init__(self, hidden_dim, pos_dim, head=4):
        super(Attention_Layer_EZPOS, self).__init__()
        self.hidden_dim = hidden_dim
        self.pos_dim = pos_dim
        if self.pos_dim > 0:
            self.Q_linear = nn.Linear(self.hidden_dim + self.pos_dim, self.hidden_dim, bias=False)
...
class DFA_PillarVFE_EZPOS(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        super().__init__(model_cfg=model_cfg)
        self.use_norm = self.model_cfg.USE_NORM
        self.with_distance = self.model_cfg.WITH_DISTANCE
        ...
        self.y_offset = self.voxel_y / 2 + point_cloud_range[1]
        self.z_offset = self.voxel_z / 2 + point_cloud_range[2]
        self.l2r_attention_layers = Attention_Layer_EZPOS(list(self.num_filters)[-1], 64)
        self.r2l_attention_layers = Attention_Layer_EZPOS(list(self.num_filters)[-1], 64)
